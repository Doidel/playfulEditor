...
(function () {
	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		var oldParams = new THREE.Vector3();
		...
		function onPointerDown( event ) {
			...
			if ( pointer.button === 0 || pointer.button === undefined ) {
				...
				if ( intersect ) {
					...
					oldParams = scope.object.geometry.parameters;
		...
		function onPointerMove( event ) {
			...
			if ( _mode == "translate" ) {
				...
				if ( scope.space == "world" || ( scope.axis && scope.axis.search("XYZ") != -1 )) {
				...
				if ( scope.space == "local" ) {
					...
					if ( scope.object.scaleBy ) {
												
						point.applyMatrix4( tempMatrix.getInverse( worldRotationMatrix ) );
						
						var scale = new THREE.Vector3( 1 + point.x / 0.5, 1 + point.y / 0.5, 1 + point.z / 0.5 );
						
						var isBox = scope.object._physijs.type == 'box';
						
						if ( scope.axis == "X" ) {
							scale.y = scale.z = isBox ? 1 : scale.x;
						}
						else if ( scope.axis == "Y" ) {
							scale.x = scale.z = isBox ? 1 : scale.y;
						}
						else if ( scope.axis == "Z" ) {
							scale.y = scale.x = isBox ? 1 : scale.z;
						}
							
						scope.object.scaleBy( scale, oldParams );
						editor.signals.objectChanged.dispatch( scope.object );
						...
						if ( scope.axis == "XYZ" ) {
							...
							console.log('sc', oldScale.y, scale);
							...
							if ( scope.axis == "X" ) scope.object.scale.x = oldScale.x * ( 1 + point.x / 0.5 );
							if ( scope.axis == "Y" ) scope.object.scale.y = oldScale.y * ( 1 + point.y / 0.5 );
							if ( scope.axis == "Z" ) scope.object.scale.z = oldScale.z * ( 1 + point.z / 0.5 );
